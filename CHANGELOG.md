...
### Changed
- `create_site` and `create_site_from_template` now stamp the new site with `by_ai: "mcp"` (persisted to the backend `sites.by_ai` column), extending the existing page-level AI-authorship marker to sites.
